...
/* ---- hand-drawn vector décor (proof of concept: tree, fountain, tent) ----
   Painted in the same storybook language as the world: bold shapes, dark
   outlines, vertical-gradient shading, white highlights, soft ground shadow.
   Each draws from a bottom-center anchor at (0,0) and animates itself. */
function vecShadow(ctx, rx) { ctx.fillStyle = 'rgba(20,35,25,.18)'; ctx.beginPath(); ctx.ellipse(0, 2, rx, rx * .28, 0, 0, Math.PI * 2); ctx.fill(); }

function drawVecTree(ctx, sec, i) {
  const sway = Math.sin(sec * 1.4 + i) * .05;
  vecShadow(ctx, 21);
  // trunk
  const tg = ctx.createLinearGradient(-5, 0, 6, 0);
  tg.addColorStop(0, '#7a4a24'); tg.addColorStop(1, '#a06a35');
  ctx.fillStyle = tg; ctx.strokeStyle = '#5a3418'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(-5, 1); ctx.lineTo(-3, -20); ctx.lineTo(3, -20); ctx.lineTo(5, 1); ctx.closePath(); ctx.fill(); ctx.stroke();
  // canopy sways as one piece atop the trunk
  ctx.save(); ctx.translate(0, -20); ctx.rotate(sway);
  const blobs = [[0, -24, 21], [-14, -12, 15], [14, -12, 15], [0, -7, 17]];
  ctx.fillStyle = '#2f7d4f'; // dark rim
  ctx.beginPath(); blobs.forEach(([bx, by, r]) => { ctx.moveTo(bx + r, by); ctx.arc(bx, by, r, 0, Math.PI * 2); }); ctx.fill();
  const cg = ctx.createLinearGradient(0, -46, 0, 6);
  cg.addColorStop(0, '#63cd86'); cg.addColorStop(1, '#3a9b5c');
  ctx.fillStyle = cg; // lighter inner fill, leaving a rim
  ctx.beginPath(); blobs.forEach(([bx, by, r]) => { ctx.moveTo(bx + r - 3, by); ctx.arc(bx, by, r - 3, 0, Math.PI * 2); }); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.4)'; // sun highlights
  ctx.beginPath(); ctx.arc(-7, -28, 4.5, 0, Math.PI * 2); ctx.arc(5, -20, 3, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}

function drawVecFountain(ctx, sec, i) {
  const t = sec + i;
  const stone = (a, b) => { const g = ctx.createLinearGradient(0, a, 0, b); g.addColorStop(0, '#d6dbe1'); g.addColorStop(1, '#98a0aa'); return g; };
  vecShadow(ctx, 23);
  ctx.strokeStyle = '#6c7480'; ctx.lineWidth = 2;
  // lower basin + water with expanding ripples
  ctx.fillStyle = stone(-16, -2); ctx.beginPath(); ctx.ellipse(0, -6, 23, 10, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#5cb8e8'; ctx.beginPath(); ctx.ellipse(0, -7, 18, 7, 0, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,.55)'; ctx.lineWidth = 1.5;
  for (let r = 0; r < 2; r++) { const k = (t * .6 + r / 2) % 1; ctx.globalAlpha = (1 - k) * .6; ctx.beginPath(); ctx.ellipse(0, -7, 4 + k * 13, 1.6 + k * 5, 0, 0, Math.PI * 2); ctx.stroke(); }
  ctx.globalAlpha = 1;
  // pillar + upper bowl
  ctx.strokeStyle = '#6c7480'; ctx.fillStyle = stone(-24, -8);
  roundRect(ctx, -5, -24, 10, 18, 3); ctx.fill(); ctx.stroke();
  ctx.fillStyle = stone(-30, -22); ctx.beginPath(); ctx.ellipse(0, -24, 12, 5, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#5cb8e8'; ctx.beginPath(); ctx.ellipse(0, -25, 9, 3.2, 0, 0, Math.PI * 2); ctx.fill();
  // spouting water arcs from the top bowl into the basin
  drawFountainWater(ctx, 0, -28, t * 1.1);
}

function drawVecTent(ctx, sec, i) {
  const H = 40, Wt = 30;
  vecShadow(ctx, 28);
  // guy-lines to pegs
  ctx.strokeStyle = 'rgba(90,70,50,.55)'; ctx.lineWidth = 1.4;
  ctx.beginPath(); ctx.moveTo(-Wt + 3, -2); ctx.lineTo(-Wt - 8, 1); ctx.moveTo(Wt - 3, -2); ctx.lineTo(Wt + 8, 1); ctx.stroke();
  ctx.strokeStyle = '#12463d'; ctx.lineWidth = 2; ctx.lineJoin = 'round';
  // right + left fabric panels, with a gently drooping base
  ctx.fillStyle = '#1c9a85';
  ctx.beginPath(); ctx.moveTo(0, -H); ctx.quadraticCurveTo(Wt * .7, -H * .3, Wt, 0); ctx.lineTo(5, 2); ctx.lineTo(0, -6); ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#26b59d';
  ctx.beginPath(); ctx.moveTo(0, -H); ctx.quadraticCurveTo(-Wt * .7, -H * .3, -Wt, 0); ctx.lineTo(-5, 2); ctx.lineTo(0, -6); ctx.closePath(); ctx.fill(); ctx.stroke();
  // gold accent stripe along each eave
  ctx.strokeStyle = '#ffb627'; ctx.lineWidth = 2.5;
  ctx.beginPath(); ctx.moveTo(-Wt + 4, -3); ctx.quadraticCurveTo(-Wt * .55, -H * .28, -1, -H + 5); ctx.moveTo(Wt - 4, -3); ctx.quadraticCurveTo(Wt * .55, -H * .28, 1, -H + 5); ctx.stroke();
  // arched door with dark interior
  ctx.fillStyle = '#0e3b33';
  ctx.beginPath(); ctx.moveTo(-8, 1); ctx.quadraticCurveTo(0, -22, 8, 1); ctx.closePath(); ctx.fill();
  ctx.strokeStyle = 'rgba(255,255,255,.35)'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.moveTo(-8, 1); ctx.quadraticCurveTo(0, -22, 8, 1); ctx.stroke();
  // ridge pole + a little pennant fluttering on top
  ctx.strokeStyle = '#7a5a34'; ctx.lineWidth = 2.5; ctx.beginPath(); ctx.moveTo(0, -H); ctx.lineTo(0, -H - 11); ctx.stroke();
  const fl = Math.sin(sec * 4 + i) * 3;
  ctx.fillStyle = '#ff6b5b';
  ctx.beginPath(); ctx.moveTo(0, -H - 11); ctx.lineTo(13, -H - 8 + fl); ctx.lineTo(0, -H - 4); ctx.closePath(); ctx.fill();
}

function drawVecFlower(ctx, sec, i) {
  const sway = Math.sin(sec * 1.8 + i) * .12;
  vecShadow(ctx, 10);
  ctx.save(); ctx.rotate(sway);
  ctx.strokeStyle = '#3a9b5c'; ctx.lineWidth = 3; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0, 0); ctx.quadraticCurveTo(2, -12, 0, -24); ctx.stroke();
  ctx.fillStyle = '#43a869';
  ctx.beginPath(); ctx.ellipse(-6, -12, 5, 2.6, -.6, 0, Math.PI * 2); ctx.fill();
  ctx.beginPath(); ctx.ellipse(6, -16, 5, 2.6, .6, 0, Math.PI * 2); ctx.fill();
  ctx.translate(0, -26);
  ctx.fillStyle = '#ff8fb0'; ctx.strokeStyle = '#e06a92'; ctx.lineWidth = 1.5;
  for (let p = 0; p < 5; p++) {
    const a = p / 5 * Math.PI * 2 - Math.PI / 2;
    ctx.beginPath(); ctx.ellipse(Math.cos(a) * 7, Math.sin(a) * 7, 5.5, 4, a, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  }
  ctx.fillStyle = '#ffd75e'; ctx.beginPath(); ctx.arc(0, 0, 4.5, 0, Math.PI * 2); ctx.fill();
  ctx.strokeStyle = '#e0a020'; ctx.stroke();
  ctx.restore();
}
function drawVecTulip(ctx, sec, i) {
  const sway = Math.sin(sec * 1.7 + i) * .1;
  vecShadow(ctx, 9);
  ctx.save(); ctx.rotate(sway);
  ctx.strokeStyle = '#3a9b5c'; ctx.lineWidth = 3; ctx.lineCap = 'round';
  ctx.beginPath(); ctx.moveTo(0, 0); ctx.lineTo(0, -22); ctx.stroke();
  ctx.fillStyle = '#43a869';
  ctx.beginPath(); ctx.moveTo(0, -4); ctx.quadraticCurveTo(-12, -12, -6, -24); ctx.quadraticCurveTo(-3, -12, 0, -8); ctx.fill();
  ctx.beginPath(); ctx.moveTo(0, -6); ctx.quadraticCurveTo(12, -14, 6, -26); ctx.quadraticCurveTo(3, -14, 0, -10); ctx.fill();
  ctx.translate(0, -22);
  ctx.fillStyle = '#e8556f'; ctx.strokeStyle = '#b83a55'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.moveTo(-8, 2); ctx.quadraticCurveTo(-9, -12, 0, -12); ctx.quadraticCurveTo(9, -12, 8, 2); ctx.quadraticCurveTo(0, 6, -8, 2); ctx.fill(); ctx.stroke();
  ctx.strokeStyle = 'rgba(255,255,255,.5)'; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(-3, -11); ctx.lineTo(-3.5, 3); ctx.moveTo(3, -11); ctx.lineTo(3.5, 3); ctx.stroke();
  ctx.restore();
}
function drawVecBush(ctx, sec, i) {
  const sway = Math.sin(sec * 1.5 + i) * .04;
  vecShadow(ctx, 20);
  ctx.save(); ctx.rotate(sway);
  const blobs = [[-11, -10, 12], [11, -10, 12], [0, -16, 14]];
  ctx.fillStyle = '#2f7d4f';
  ctx.beginPath(); blobs.forEach(([bx, by, r]) => { ctx.moveTo(bx + r, by); ctx.arc(bx, by, r, 0, Math.PI * 2); }); ctx.fill();
  const cg = ctx.createLinearGradient(0, -30, 0, 2);
  cg.addColorStop(0, '#5cc47f'); cg.addColorStop(1, '#3a9b5c');
  ctx.fillStyle = cg;
  ctx.beginPath(); blobs.forEach(([bx, by, r]) => { ctx.moveTo(bx + r - 2.5, by); ctx.arc(bx, by, r - 2.5, 0, Math.PI * 2); }); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.35)';
  ctx.beginPath(); ctx.arc(-8, -16, 3, 0, Math.PI * 2); ctx.arc(3, -20, 2.5, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#ff6b5b';
  ctx.beginPath(); ctx.arc(-4, -8, 1.8, 0, Math.PI * 2); ctx.arc(8, -12, 1.8, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}
function drawVecPine(ctx, sec, i) {
  const sway = Math.sin(sec * 1.3 + i) * .03;
  vecShadow(ctx, 16);
  ctx.fillStyle = '#7a4a24'; ctx.strokeStyle = '#5a3418'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.rect(-3, -8, 6, 9); ctx.fill(); ctx.stroke();
  ctx.save(); ctx.translate(0, -8); ctx.rotate(sway);
  const tier = (y, w, h) => {
    ctx.fillStyle = '#2c7a4a';
    ctx.beginPath(); ctx.moveTo(0, y - h - 2); ctx.lineTo(w + 2, y); ctx.lineTo(-w - 2, y); ctx.closePath(); ctx.fill();
    const g = ctx.createLinearGradient(0, y - h, 0, y);
    g.addColorStop(0, '#4fb377'); g.addColorStop(1, '#328a55');
    ctx.fillStyle = g;
    ctx.beginPath(); ctx.moveTo(0, y - h); ctx.lineTo(w, y); ctx.lineTo(-w, y); ctx.closePath(); ctx.fill();
  };
  tier(0, 17, 16); tier(-12, 14, 15); tier(-23, 10, 14);
  ctx.fillStyle = 'rgba(255,255,255,.5)';
  ctx.beginPath(); ctx.arc(0, -35, 2.5, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}
function drawVecPlanter(ctx, sec, i) {
  const sway = Math.sin(sec * 1.6 + i) * .05;
  vecShadow(ctx, 15);
  ctx.save(); ctx.translate(0, -14); ctx.rotate(sway);
  const blobs = [[-7, -6, 9], [7, -6, 9], [0, -12, 10]];
  ctx.fillStyle = '#2f7d4f';
  ctx.beginPath(); blobs.forEach(([bx, by, r]) => { ctx.moveTo(bx + r, by); ctx.arc(bx, by, r, 0, Math.PI * 2); }); ctx.fill();
  ctx.fillStyle = '#4fb377';
  ctx.beginPath(); blobs.forEach(([bx, by, r]) => { ctx.moveTo(bx + r - 2.5, by); ctx.arc(bx, by, r - 2.5, 0, Math.PI * 2); }); ctx.fill();
  ctx.restore();
  const pg = ctx.createLinearGradient(0, -16, 0, 0);
  pg.addColorStop(0, '#d98550'); pg.addColorStop(1, '#b5623a');
  ctx.fillStyle = pg; ctx.strokeStyle = '#8f4a2c'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(-11, -16); ctx.lineTo(11, -16); ctx.lineTo(8, 0); ctx.lineTo(-8, 0); ctx.closePath(); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#e09460';
  roundRect(ctx, -12, -18, 24, 5, 2); ctx.fill(); ctx.stroke();
}
function drawVecMushroom(ctx, sec, i) {
  const bob = Math.sin(sec * 2 + i) * 1;
  vecShadow(ctx, 12);
  ctx.save(); ctx.translate(0, bob);
  const sg = ctx.createLinearGradient(-5, 0, 6, 0);
  sg.addColorStop(0, '#f3ead6'); sg.addColorStop(1, '#fffaf0');
  ctx.fillStyle = sg; ctx.strokeStyle = '#d8c9a8'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.moveTo(-5, 0); ctx.quadraticCurveTo(-6, -12, -4, -14); ctx.lineTo(4, -14); ctx.quadraticCurveTo(6, -12, 5, 0); ctx.closePath(); ctx.fill(); ctx.stroke();
  const cg = ctx.createLinearGradient(0, -30, 0, -12);
  cg.addColorStop(0, '#ff6b5b'); cg.addColorStop(1, '#d83f34');
  ctx.fillStyle = cg; ctx.strokeStyle = '#b23228'; ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(-15, -13); ctx.quadraticCurveTo(0, -34, 15, -13); ctx.quadraticCurveTo(0, -20, -15, -13); ctx.fill(); ctx.stroke();
  ctx.fillStyle = 'rgba(255,255,255,.9)';
  [[-7, -18, 2.6], [4, -20, 3], [9, -15, 2], [-1, -24, 2.2]].forEach(([x, y, r]) => { ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill(); });
  ctx.restore();
}
function drawVecBalloon(ctx, sec, i) {
  const ph = sec * 1.6 + i;
  const by = -38 - (Math.sin(ph) * .5 + .5) * 8;
  ctx.strokeStyle = 'rgba(120,120,140,.7)'; ctx.lineWidth = 1.4;
  ctx.beginPath(); ctx.moveTo(0, 0); ctx.quadraticCurveTo(Math.sin(ph) * 6, by / 2, Math.sin(ph) * 2, by + 12); ctx.stroke();
  ctx.save(); ctx.translate(Math.sin(ph) * 2, by); ctx.rotate(Math.sin(ph) * .06);
  const g = ctx.createRadialGradient(-4, -4, 2, 0, 0, 15);
  g.addColorStop(0, '#ff8f8f'); g.addColorStop(1, '#e23b3b');
  ctx.fillStyle = g; ctx.strokeStyle = '#b52a2a'; ctx.lineWidth = 1.5;
  ctx.beginPath(); ctx.ellipse(0, 0, 12, 14, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#e23b3b';
  ctx.beginPath(); ctx.moveTo(-3, 13); ctx.lineTo(3, 13); ctx.lineTo(0, 17); ctx.closePath(); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.5)';
  ctx.beginPath(); ctx.ellipse(-4, -5, 3, 4.5, -.4, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
}
function drawVecBench(ctx, sec, i) {
  vecShadow(ctx, 20);
  const wood = (a, b) => { const g = ctx.createLinearGradient(0, a, 0, b); g.addColorStop(0, '#b07a44'); g.addColorStop(1, '#8a5a30'); return g; };
  ctx.strokeStyle = '#5a3418'; ctx.lineWidth = 1.5;
  ctx.fillStyle = '#6f4a28';
  ctx.fillRect(-15, -10, 3.5, 10); ctx.fillRect(12, -10, 3.5, 10);
  ctx.fillStyle = wood(-14, -9);
  roundRect(ctx, -18, -14, 36, 5, 2); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#6f4a28';
  ctx.fillRect(-16, -26, 3, 14); ctx.fillRect(13, -26, 3, 14);
  ctx.fillStyle = wood(-26, -16);
  roundRect(ctx, -18, -26, 36, 4, 2); ctx.fill(); ctx.stroke();
  roundRect(ctx, -18, -20, 36, 4, 2); ctx.fill(); ctx.stroke();
}
function drawVecFlag(ctx, sec, i) {
  vecShadow(ctx, 8);
  ctx.fillStyle = '#8a5a34'; ctx.fillRect(-1.5, -40, 3, 41);
  ctx.fillStyle = '#ffd75e'; ctx.beginPath(); ctx.arc(0, -40, 3, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#e23b3b'; ctx.strokeStyle = '#b52a2a'; ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(1.5, -38);
  const seg = 6;
  for (let s = 0; s <= seg; s++) { const x = 1.5 + (s / seg) * 24, y = -38 + Math.sin(sec * 4 + s * .9 + i) * 2.2; ctx.lineTo(x, y); }
  for (let s = seg; s >= 0; s--) { const x = 1.5 + (s / seg) * 24, y = -26 + Math.sin(sec * 4 + s * .9 + i) * 2.2; ctx.lineTo(x, y); }
  ctx.closePath(); ctx.fill(); ctx.stroke();
}
function drawVecLantern(ctx, sec, i) {
  drawLanternGlow(ctx, 7, -30, sec + i);
  vecShadow(ctx, 10);
  ctx.fillStyle = '#6f7681'; ctx.strokeStyle = '#4c525b'; ctx.lineWidth = 1.5;
  ctx.fillRect(-2.5, -30, 5, 30); ctx.strokeRect(-2.5, -30, 5, 30);
  ctx.beginPath(); ctx.moveTo(0, -30); ctx.lineTo(0, -38); ctx.lineTo(7, -38); ctx.stroke();
  const fl = .8 + Math.sin(sec * 6 + i) * .15;
  const lg = ctx.createLinearGradient(0, -37, 0, -23);
  lg.addColorStop(0, `rgba(255,214,130,${fl})`); lg.addColorStop(1, `rgba(255,168,70,${fl})`);
  ctx.save(); ctx.translate(7, -30);
  ctx.fillStyle = lg; ctx.strokeStyle = '#b5701f'; ctx.lineWidth = 1.5;
  roundRect(ctx, -6, -6, 12, 13, 3); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#3a3348'; ctx.fillRect(-5, -8, 10, 2.5); ctx.fillRect(-5, 6, 10, 2.5);
  ctx.restore();
}
function drawVecStar(ctx, sec, i) {
  const spin = sec * .5 + i, hover = -26 + Math.sin(sec * 1.5 + i) * 3;
  const g = ctx.createRadialGradient(0, hover, 2, 0, hover, 22);
  g.addColorStop(0, 'rgba(255,226,122,.5)'); g.addColorStop(1, 'rgba(255,226,122,0)');
  ctx.fillStyle = g; ctx.beginPath(); ctx.arc(0, hover, 22, 0, Math.PI * 2); ctx.fill();
  vecShadow(ctx, 7);
  ctx.save(); ctx.translate(0, hover); ctx.rotate(spin);
  ctx.fillStyle = '#ffd23f'; ctx.strokeStyle = '#e0a020'; ctx.lineWidth = 2;
  drawStar(ctx, 0, 0, 5, 13, 6); ctx.fill(); ctx.stroke();
  ctx.fillStyle = 'rgba(255,255,255,.6)'; ctx.beginPath(); ctx.arc(-3, -3, 2.5, 0, Math.PI * 2); ctx.fill();
  ctx.restore();
  const t = (sec * 1.5 + i) % 2.2;
  if (t < 1) { ctx.save(); ctx.globalAlpha = (1 - t) * .9; ctx.fillStyle = '#fff'; drawStar(ctx, 12, hover - 12, 4, 3, 1.2); ctx.fill(); ctx.restore(); }
}
function drawVecPond(ctx, sec, i) {
  ctx.fillStyle = 'rgba(20,35,25,.16)'; ctx.beginPath(); ctx.ellipse(0, -1, 25, 12, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#7a6a4a';
  ctx.beginPath(); ctx.ellipse(0, -4, 24, 11.5, 0, 0, Math.PI * 2); ctx.fill();
  const wg = ctx.createLinearGradient(0, -14, 0, 6);
  wg.addColorStop(0, '#6fd0e8'); wg.addColorStop(1, '#3a94c8');
  ctx.fillStyle = wg;
  ctx.beginPath(); ctx.ellipse(0, -5, 20, 9, 0, 0, Math.PI * 2); ctx.fill();
  drawPondRipple(ctx, -3, -5, sec + i);
  ctx.fillStyle = '#3a9b5c';
  ctx.beginPath(); ctx.ellipse(9, -3, 6, 3, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = '#ff8fb0'; ctx.beginPath(); ctx.arc(9, -4, 2, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.5)';
  ctx.beginPath(); ctx.ellipse(-8, -8, 5, 2, -.3, 0, Math.PI * 2); ctx.fill();
}
function drawVecRainbow(ctx, sec, i) {
  const bands = ['#ff5b5b', '#ff9e3d', '#ffd23f', '#4fb377', '#4a90e2', '#9b5de5'];
  ctx.save(); ctx.globalAlpha = .82 + Math.sin(sec * 1.5 + i) * .12;
  ctx.lineWidth = 4; ctx.lineCap = 'round';
  bands.forEach((c, b) => { ctx.strokeStyle = c; ctx.beginPath(); ctx.arc(0, 0, 30 - b * 4, Math.PI, 0); ctx.stroke(); });
  ctx.globalAlpha = 1; ctx.fillStyle = '#ffffff';
  [-30, 30].forEach(cx => { ctx.beginPath(); ctx.arc(cx - 5, 0, 6, 0, Math.PI * 2); ctx.arc(cx + 4, 0, 7, 0, Math.PI * 2); ctx.arc(cx, -4, 6, 0, Math.PI * 2); ctx.fill(); });
  ctx.restore();
}
function drawVecCastle(ctx, sec, i) {
  vecShadow(ctx, 24);
  const sand = (a, b) => { const g = ctx.createLinearGradient(0, a, 0, b); g.addColorStop(0, '#f0d69a'); g.addColorStop(1, '#d4b06a'); return g; };
  ctx.strokeStyle = '#b58f4f'; ctx.lineWidth = 2;
  const tower = (x, w, h) => {
    ctx.fillStyle = sand(-h, 0);
    ctx.beginPath(); ctx.rect(x - w / 2, -h, w, h); ctx.fill(); ctx.stroke();
    ctx.fillStyle = '#e6c884';
    for (let c = 0; c < 3; c++) ctx.fillRect(x - w / 2 + c * (w / 3), -h - 4, w / 3 - 1.5, 4);
  };
  tower(-16, 14, 30); tower(16, 14, 30);
  ctx.fillStyle = sand(-40, 0);
  ctx.beginPath(); ctx.rect(-11, -40, 22, 40); ctx.fill(); ctx.stroke();
  ctx.fillStyle = '#e6c884';
  for (let c = 0; c < 4; c++) ctx.fillRect(-11 + c * 5.5, -44, 4, 4);
  ctx.fillStyle = '#7a5a34';
  ctx.beginPath(); ctx.moveTo(-5, 0); ctx.lineTo(-5, -8); ctx.quadraticCurveTo(0, -13, 5, -8); ctx.lineTo(5, 0); ctx.closePath(); ctx.fill();
  const fw = (fx, fh) => {
    ctx.strokeStyle = '#8a5a34'; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(fx, -fh); ctx.lineTo(fx, -fh - 10); ctx.stroke();
    const fl = Math.sin(sec * 4 + fx + i) * 2;
    ctx.fillStyle = '#4a90e2';
    ctx.beginPath(); ctx.moveTo(fx, -fh - 10); ctx.lineTo(fx + 8, -fh - 8 + fl); ctx.lineTo(fx, -fh - 6); ctx.closePath(); ctx.fill();
  };
  fw(-16, 34); fw(16, 34); fw(0, 44);
}

const HOME_VEC = {
  tree: drawVecTree, fountain: drawVecFountain, tent: drawVecTent, flower: drawVecFlower,
  tulip: drawVecTulip, bush: drawVecBush, pine: drawVecPine, planter: drawVecPlanter,
  mushroom: drawVecMushroom, balloon: drawVecBalloon, bench: drawVecBench, flag: drawVecFlag,
  lantern: drawVecLantern, star: drawVecStar, pond: drawVecPond, rainbow: drawVecRainbow, castle: drawVecCastle,
};

function homeHalo(ctx, x, y, i, sec) {
  ...
  ctx.fillStyle = rg; ctx.beginPath(); ctx.arc(x, y, 36, 0, Math.PI * 2); ctx.fill();
}
...
function drawDecorItem(ctx, d, p, i, sec) {
  ...
  // hand-drawn vector décor takes over for its items (self-contained: shadow, body, animation)
  const vec = HOME_VEC[d.id];
  if (vec) {
    if (HOME_LIVE[d.id]) homeHalo(ctx, p.x, p.y - 6, i, sec);
    ctx.save(); ctx.translate(p.x, p.y + 8); vec(ctx, sec, i); ctx.restore();
    return;
  }
