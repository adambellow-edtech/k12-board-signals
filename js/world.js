...
const BUILDINGS = [
  ...
  { id: 'math',   name: 'Math Trailhead',      sub: 'Breakout Math',   x: 1180, y: 400,  w: 190, h: 140 },
...
const BUSHES = [
  [340, 420], [520, 250], [1300, 560], [1030, 640], [640, 760], [300, 780], [1360, 880], [880, 240],
];
const ROCKS = [[460, 700], [1120, 300], [980, 960], [220, 560]];
...
function stepPlayer() {
  ...
  state.pos.x = Math.max(130, Math.min(WORLD.w - 130, state.pos.x));
  state.pos.y = Math.max(240, Math.min(WORLD.h - 100, state.pos.y));
...
function drawWorld(sec) {
  ...
  sky.addColorStop(0, '#79cdec'); sky.addColorStop(.6, '#a5e0f2'); sky.addColorStop(1, '#cdeef8');
  ctx.fillStyle = sky;
  ...
  // ---- ocean ----
  const sea = ctx.createLinearGradient(0, 100, 0, WORLD.h);
  sea.addColorStop(0, '#2d9bcb'); sea.addColorStop(1, '#45b7e0');
  ctx.fillStyle = sea;
  roundRect(ctx, 14, 120, WORLD.w - 28, WORLD.h - 148, 100); ctx.fill();
  // light bands in water
  ctx.fillStyle = 'rgba(255,255,255,.06)';
  for (let i = 0; i < 4; i++) {
    roundRect(ctx, 40 + i * 12, 150 + i * 26, WORLD.w - 80 - i * 24, WORLD.h - 210 - i * 50, 90); ctx.fill();
  }
  // waves
  ctx.strokeStyle = 'rgba(255,255,255,.4)'; ctx.lineWidth = 3; ctx.lineCap = 'round';
  for (let i = 0; i < 10; i++) {
    const wx = 110 + (i * 217) % (WORLD.w - 240), wy = 160 + (i * 313) % (WORLD.h - 250);
    const off = Math.sin(sec * 1.5 + i) * 5;
    ctx.beginPath(); ctx.arc(wx + off, wy, 11, Math.PI * .12, Math.PI * .88); ctx.stroke();
  }

  // ---- island: foam, beach, grass ----
  ctx.fillStyle = 'rgba(255,255,255,.75)';
  roundRect(ctx, 58 - Math.sin(sec * 1.1) * 2, 178, WORLD.w - 116 + Math.sin(sec * 1.1) * 4, WORLD.h - 252, 86); ctx.fill();
  const sand = ctx.createLinearGradient(0, 180, 0, WORLD.h);
  sand.addColorStop(0, '#f2e0ad'); sand.addColorStop(1, '#e7cd92');
  ctx.fillStyle = sand;
  roundRect(ctx, 66, 186, WORLD.w - 132, WORLD.h - 264, 80); ctx.fill();
  // wet sand line
  ctx.strokeStyle = 'rgba(160,128,70,.28)'; ctx.lineWidth = 5;
  roundRect(ctx, 72, 192, WORLD.w - 144, WORLD.h - 276, 76); ctx.stroke();

  const grass = ctx.createRadialGradient(WORLD.w / 2, WORLD.h * .45, 120, WORLD.w / 2, WORLD.h * .55, 900);
  grass.addColorStop(0, '#8ed172'); grass.addColorStop(.7, '#74bd5c'); grass.addColorStop(1, '#5da84c');
  ctx.fillStyle = grass;
  roundRect(ctx, 92, 212, WORLD.w - 184, WORLD.h - 316, 66); ctx.fill();
  // mowed light bands
  ctx.save();
  roundRect(ctx, 92, 212, WORLD.w - 184, WORLD.h - 316, 66); ctx.clip();
  ...
  ctx.lineWidth = 2;
  ctx.strokeStyle = 'rgba(170,132,80,.55)';
  ...
  TUFTS.forEach(([tx, ty], i) => {
    if (Math.hypot(tx - 800, ty - 570) < 120) return;
  ...
  FLOWERS.forEach(([fx, fy, c], i) => {
    if (Math.hypot(fx - 800, fy - 570) < 125) return;
  ...
  const drawables = [
    ...
    ...BUSHES.map(([bx, by]) => ({ y: by, fn: () => drawBush(ctx, bx, by, sec) })),
    {
      y: state.pos.y,
      fn: () => {
