...
function drawHomePlot(ctx, sec) {
  ...
  ctx.save();
  // grassy pad
  ctx.fillStyle = 'rgba(122,206,156,.55)';
  roundRect(ctx, x0 - 18, y0 - 14, w + 36, h + 34, 30); ctx.fill();
  ctx.strokeStyle = 'rgba(28,154,133,.45)'; ctx.lineWidth = 3;
  roundRect(ctx, x0 - 18, y0 - 14, w + 36, h + 34, 30); ctx.stroke();
  // stone path from the front gate into the yard
  ctx.strokeStyle = 'rgba(230,217,178,.8)'; ctx.lineWidth = 13; ctx.lineCap = 'round';
  ctx.setLineDash([2, 15]);
  ctx.beginPath(); ctx.moveTo(P.cx, y0 + h + 16); ctx.lineTo(P.cx, P.cy); ctx.stroke();
  ctx.setLineDash([]);
  drawFence(ctx, x0 - 18, y0 - 14, w + 36, h + 34);
  drawHomeSign(ctx, sec);
  // decor, painted back-to-front so nearer items overlap farther ones
  const placed = (state.home && state.home.placed) || {};
  ...
  for (const i of ids) {
    ...
    const p = homeCellXY(i);
    const bob = Math.sin(sec * 1.6 + i * 1.3) * 2;
    if (HOME_LIVE[d.id]) {
      const glow = .3 + Math.sin(sec * 3 + i) * .16;
      const g = ctx.createRadialGradient(p.x, p.y - 2, 2, p.x, p.y - 2, 34);
      g.addColorStop(0, `rgba(255,226,122,${glow})`); g.addColorStop(1, 'rgba(255,226,122,0)');
      ctx.fillStyle = g; ctx.beginPath(); ctx.arc(p.x, p.y - 2, 34, 0, Math.PI * 2); ctx.fill();
    }
    ctx.fillStyle = 'rgba(0,40,20,.16)';
    ctx.beginPath(); ctx.ellipse(p.x, p.y + 16, 16, 6, 0, 0, Math.PI * 2); ctx.fill();
    const sz = 44;
    ctx.drawImage(emojiSprite(d.icon), p.x - sz / 2, p.y - sz + 8 + bob, sz, sz);
  }
  ...
  world.homeFx.forEach(pt => {
    const k = (sec - pt.t) / .8;
    const px = pt.x + pt.vx * k, py = pt.y + pt.vy * k + 60 * k * k;
    ctx.save(); ctx.globalAlpha = 1 - k; ctx.fillStyle = pt.color;
    ctx.beginPath(); ctx.arc(px, py, 3.5 * (1 - k) + 1, 0, Math.PI * 2); ctx.fill(); ctx.restore();
  });
  ctx.restore();
}
...
function drawFence(ctx, x, y, w, h) {
  ...
  for (const ry of [y, y + h]) {
    ...
    for (let px = x + 8; px <= x + w - 8; px += gap) {
      if (ry === y + h && Math.abs(px - (x + w / 2)) < 26) continue; // leave a front gate gap
      ctx.fillStyle = 'rgba(255,255,255,.92)';
      ctx.beginPath();
      ctx.moveTo(px - pw / 2, ry + 6); ctx.lineTo(px - pw / 2, ry - 10);
      ctx.lineTo(px, ry - 16); ctx.lineTo(px + pw / 2, ry - 10);
