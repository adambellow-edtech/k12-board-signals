...
function renderArcade(title, body) {
  ...
  body.innerHTML = `
    ...
    <div class="card game-card">
      <div class="game-info">
        <h3>Signal 🔔</h3>
        <p class="story">Watch the glowing pattern, then repeat it back. Each round adds one — how far can you echo?</p>
      </div>
      <div class="game-side">
        <button class="btn-big" id="sig-go" ${state.arcadeMin < 5 ? 'disabled' : ''}>${state.arcadeMin < 5 ? 'Need 5 min' : 'Play (−5 min)'}</button>
      </div>
    </div>`;
  ...
  const sg = document.getElementById('sig-go');
  if (sg && state.arcadeMin >= 5) sg.onclick = () => { closeBuildingModal(); startSignal(); };
}

/* Signal — a Simon-style pattern-memory arcade game. Watch the glowing sequence,
   repeat it, each round adds a step. Pure arcade juice. */
const SIGNAL_PADS = [
  { c: '#ff5b6a', lit: '#ffd3d8', tone: 330 },
  { c: '#2ec4b6', lit: '#c8fff7', tone: 415 },
  { c: '#4c9aff', lit: '#d6e9ff', tone: 494 },
  { c: '#ffb627', lit: '#fff0cc', tone: 660 },
];
function startSignal() {
  ...
  modal.classList.add('open');
  let seq = [], input = [], round = 0, playerTurn = false, over = false;

  grid.innerHTML = '';
  const pads = SIGNAL_PADS.map((p, i) => {
    const b = document.createElement('button');
    b.className = 'sig-pad'; b.style.setProperty('--pc', p.c); b.style.setProperty('--pl', p.lit);
    b.onclick = () => onPad(i);
    grid.appendChild(b);
    return b;
  });
  const flash = (i, ms = 380) => {
    pads[i].classList.add('lit'); tone(SIGNAL_PADS[i].tone, .28, 'sine', .07);
    setTimeout(() => pads[i].classList.remove('lit'), ms);
  };
  const playSeq = () => {
    playerTurn = false; status.textContent = `Round ${round} — watch closely…`;
    seq.forEach((i, k) => setTimeout(() => flash(i), 650 + k * 620));
    setTimeout(() => { playerTurn = true; input = []; status.textContent = `Round ${round} — your turn! (${seq.length})`; }, 650 + seq.length * 620 + 200);
  };
  const nextRound = () => {
    round++; seq.push((Math.random() * 4) | 0); playSeq();
  };
  const onPad = (i) => {
    if (!playerTurn || over) return;
    flash(i, 220); input.push(i);
    if (input[input.length - 1] !== seq[input.length - 1]) return gameOver();
    if (input.length === seq.length) {
      playerTurn = false; blip(900);
      status.textContent = `Nice! Round ${round} cleared 🎉`;
      setTimeout(nextRound, 800);
    }
  };
  const gameOver = () => {
    over = true; playerTurn = false; buzz();
    const reached = round - 1;
    const reward = Math.min(20, 3 + reached * 2);
    grid.classList.add('shake');
    setTimeout(() => {
      modal.classList.remove('open'); grid.classList.remove('shake');
      if (reached >= 1) { grant({ keys: reward, xp: reached * 4 }); fanfare(); confetti(); toast(`Signal: reached round ${reached}! +${reward} 🔑`); }
      else toast('Signal: give it another go — you’ll get the rhythm!');
    }, 900);
  };
  document.getElementById('signal-quit').onclick = () => { over = true; modal.classList.remove('open'); };
  if (typeof window !== 'undefined') window.__signal = { seq, ready: () => playerTurn, tap: onPad };
